...
window.Shader2 = window.classes.Shader2 =
class Shader2 extends Shader          // THE DEFAULT SHADER: This uses the Phong Reflection Model, with optional Gouraud shading. 
                                           // Wikipedia has good defintions for these concepts.  Subclasses of class Shader each store 
                                           // and manage a complete GPU program.  This particular one is a big "master shader" meant to 
                                           // handle all sorts of lighting situations in a configurable way. 
                                           // Phong Shading is the act of determining brightness of pixels via vector math.  It compares
...
{ material( color, properties )     // Define an internal class "Material" that stores the standard settings found in Phong lighting.
  ...
  vertex_glsl_code()           // ********* VERTEX SHADER *********
    { return `
        ...
        void main()
        { gl_Position = projection_camera_model_transform * vec4(object_space_pos, 1.0);     // The vertex's final resting place (in NDCS).
          shadowPos = texUnitConverter * lightMVP * vec4(object_space_pos, 1.0);
          N = normalize( inverse_transpose_modelview * normal );                             // The final normal vector in screen space.
          f_tex_coord = tex_coord;                                         // Directly use original texture coords and interpolate between.
    ...
    {                            // A fragment is a pixel that's overlapped by the current triangle.
      ...
      return `
        ...
        { 
          ...
          vec3 uColor = vec3(0.40, 0.26, 0.13);
          if( GOURAUD || COLOR_NORMALS )    // Do smooth "Phong" shading unless options like "Gouraud mode" are wanted instead.
          { gl_FragColor = VERTEX_COLOR;    // Otherwise, we already have final colors to smear (interpolate) across vertices.            
            return;
          }                                 // If we get this far, calculate Smooth "Phong" Shading as opposed to Gouraud Shading.
          ...
          if( USE_TEXTURE ) {
            //my stuff
             ...
             amountInLight /= 9.0;
            //float texelDepth = decodeFloat(texture2D(texture, fragmentDepth.xy));
//             float texelDepth = texture2D(texture, fragmentDepth.xy).z;
//             if (fragmentDepth.z < texelDepth){
//               amountInLight = 1.0;
...
//             }
            ...
            gl_FragColor = vec4(amountInLight * shapeColor.xyz, 1.0);
            //gl_FragColor = vec4(texelDepth-fragmentDepth.z,texelDepth-fragmentDepth.z,texelDepth-fragmentDepth.z,1.0);
            //gl_FragColor = lightMViewMatrix[1];
  ...
  update_matrices( g_state, model_transform, gpu, gl )                                    // Helper function for sending matrices to GPU.
    {                                                   // (PCM will mean Projection * Camera * Model)
      ...
      gl.uniformMatrix3fv( gpu.inverse_transpose_modelview_loc,       false, Mat.flatten_2D_to_1D( inv_CM              ) );
      //light matrices
      //const A = Mat4.orthographic( -40, 40, -40, 40, -10, 20 );
      //const B = Mat4.look_at(Vec.of(0, 10, 1), Vec.of(0, 0, 0), Vec.of(0, 1, 0));
      let [ LP, LC, LM, LR ]    = [ Mat4.orthographic( -40, 40, -40, 40, -10, 20 ), 
                        Mat4.look_at( Vec.of( 0,10,1 ), Vec.of( 0,0,0 ), Vec.of( 0,1,0 ) ), model_transform,
                        Mat4.rotation(Math.PI, [1,0,0]) ],
            LCM     =      LC.times(  LM ),
            LPCM    =      LP.times( LCM ),
            LPCMR   =      LR.times( LPCM );
//       console.log(A);
//       console.log(B);
      ...
      //gl.uniformMatrix4fv( gpu.lightMViewMatrix_loc, false, Mat.flatten_2D_to_1D( B.transposed() ) );       
    }
}

// light shader
window.Shader1 = window.classes.Shader1 =
class Shader1 extends Shader             // Subclasses of Shader each store and manage a complete GPU program.  This Shader is 
{                                             // the simplest example of one.  It samples pixels from colors that are directly assigned 
  material() { return { shader: this } }      // to the vertices.  Materials here are minimal, without any settings.
  map_attribute_name_to_buffer_name( name )        // The shader will pull single entries out of the vertex arrays, by their data fields'
    {                                              // names.  Map those names onto the arrays we'll pull them from.  This determines
    ...
    // Define how to synchronize our JavaScript's variables to the GPU's:
  update_GPU( g_state, model_transform, material, gpu = this.g_addrs, gl = this.gl )
      { const [ P, C, M ] = [ Mat4.orthographic( -40, 40, -40, 40, -10, 20 ), Mat4.look_at( Vec.of( 0,10,1 ), Vec.of( 0,0,0 ), Vec.of( 0,1,0 ) ), model_transform ],
                      PCM = P.times( C ).times( M );
        gl.uniformMatrix4fv( gpu.projection_camera_model_transform_loc, false, Mat.flatten_2D_to_1D( PCM.transposed() ) );
      }
  shared_glsl_code()            // ********* SHARED CODE, INCLUDED IN BOTH SHADERS *********
